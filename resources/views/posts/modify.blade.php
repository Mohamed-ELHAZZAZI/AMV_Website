<x-layout>
        <div class="w-11/12 md:w-4/5 max-w-2xl mx-auto mt-3 mb-5">
              ...
              <form action="/posts/store" id="postForm" method="POST" class="w-full h-full flex flex-col gap-5" enctype="multipart/form-data">
                ...
                <div class="flex gap-2" id="tags-container">
                    ...
                    @foreach ($tags as $key => $tag)
                      <span class="h-9 px-2 bg-dark-400 flex items-center justify-center rounded-lg gap-2 " data-index="{{$key}}">
                        {{$tag}}<i class="fa-solid fa-x text-xs cursor-pointer close"></i>
                      </span>
                    @endforeach
                </div>
                  <input
                    ...
                    id="TagsInput"
                  />
                  <input type = 'hidden' value = '{{$post->tags}}' name='tags' id="tags-input-value">
                </div>
                </div>
          ...
          <script>
            var tags_input_value = $('#tags-input-value').val();
            var tagsARRAY=[];
            tagsARRAY = tags_input_value.split(',')
